...
const headerButton =
  'h-8 shrink-0 whitespace-nowrap cursor-pointer flex items-center gap-2 text-gray-200 rounded-md hover:text-white focus-visible:text-white hover:bg-[#272626] focus-visible:bg-[#272626] px-2.5 text-xs outline-none focus-visible:ring-2 focus-visible:ring-gray-400 transition-colors';
...
function Header(props: { onToggle: () => void }) {
  ...
  return (
    <header class="fixed top-0 inset-x-0 z-50 grid grid-cols-[minmax(0,1fr)_auto] md:grid-cols-[minmax(0,1fr)_auto_auto] gap-3 items-center bg-[#1a1a1a] border-b border-[#2c2d30] px-3.5 text-sm h-14 text-white select-none">
      <div class="flex items-center min-w-0 gap-2">
        ...
        <Show when={isDesktop()}>
          <button
            ...
            class="h-8 shrink-0 cursor-pointer flex items-center gap-1.5 text-gray-200 rounded-md hover:text-white hover:bg-[#272626] focus-visible:ring-2 focus-visible:ring-gray-400 px-2 text-xs transition-colors"
            title={t("Sites Overview")}
          >
            <ArrowLeft size={14} />
            <span>{t("Sites")}</span>
          </button>
          <span class="h-4 w-px shrink-0 bg-[#3e4045] mx-1" aria-hidden="true" />
        </Show>
        ...
        <span class="truncate text-gray-200 text-[13px] font-medium" title={editorConfig.siteName}>{editorConfig.siteName}</span>
      </div>

      <div class="hidden md:flex items-center justify-center gap-2">
        <A
          href="/publish"
          class="h-7 min-w-0 inline-flex items-center gap-2 rounded-md border px-2.5 text-[11.5px] font-medium transition-colors"
          classList={{
        ...
        >
          <GitBranch size={13} />
          <span class="font-mono truncate max-w-28 xl:max-w-52" title={status()?.branch ?? 'main'}>{status()?.branch ?? 'main'}</span>
          <span class="whitespace-nowrap hidden xl:inline" classList={{ '!inline': pending() > 0 }}>{pending() > 0 ? t("{{v0}} uncommitted", { v0: pending() }) : t("Committed")}</span>
        </A>
      ...
      <div class="flex justify-end items-center gap-1.5">
        <Show when={isDesktop()}>
          ...
          >
            <Code size={14} />
            <span class="hidden xl:inline">{t("Code")}</span>
          </button>
          ...
          >
            <Hammer size={14} />
            <span class="hidden xl:inline">{t("Build")}</span>
          </button>
          ...
          >
            <Folder size={14} />
            <span class="hidden xl:inline">{t("Folder")}</span>
          </button>
        </Show>
        <LanguageSwitcher />
        ...
        <A
          href="/editor"
          class="h-8 shrink-0 whitespace-nowrap inline-flex items-center gap-1.5 rounded-md bg-white text-[#1a1a1a] px-3 text-xs font-semibold hover:bg-gray-100 transition-colors"
        >
